import { House, LibraryBig, BookOpenCheck, PhoneCall, LogIn, X, Menu, Box } from  "lucide-react"
...
const HomeNav = () => {
  return (
    <nav>
        ...
        {navs.map(item => 
            item.name == "Log In" ?
            <div onClick={ () => { location.href = item.link } } className="opacity-70 hover:opacity-100 cursor-pointer flex items-center justify-center gap-0.5 min-[605px]:px-3 max-[605px]:p-1.5 max-[605px]:hover:bg-[#fee39a] font-semibold text-lg">
                <span className="mt-1 px-2">{item.icon}</span>
                <span className='whitespace-nowrap max-[605px]:w-[90px]'>{item.name}</span>
            </div>
            :
            <Link to={item.link} className="opacity-70 hover:opacity-100 cursor-pointer flex items-center justify-center gap-0.5 min-[605px]:px-3 max-[605px]:p-1.5 max-[605px]:hover:bg-[#fee39a] font-semibold text-lg">
                <span className="mt-1 px-2">{item.icon}</span>
                <span className='whitespace-nowrap max-[605px]:w-[90px]'>{item.name}</span>
            </Link>
        )}
